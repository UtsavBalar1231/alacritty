...
use crate::{OpaqueSequenceKind, Params, ParamsIter};
...
struct ProcessorState<T: Timeout> {
    ...
    /// Kind of the opaque sequence (SOS/PM/APC) currently being processed.
    ///
    /// Kept here rather than in [`Performer`] since a single sequence may span
    /// multiple `advance` calls, each of which creates a fresh [`Performer`].
...
pub trait Handler {
    ...
    fn graphics_attribute(&mut self, _pi: u16, _pa: u16, _pv: u32) {}

    /// OSC 1337 iTerm2 inline image sequence.
    ///
    /// `payload` is the raw bytes after `OSC 1337 ;`, with `;` separators
    /// within the payload already rejoined (the OSC parser splits on `;`).
    /// The handler is responsible for all iTerm2-specific parsing.
    fn osc_1337_raw(&mut self, _payload: &[u8]) {}
    ...
    /// Start of a device control string (DCS).
    ///
    /// Payload bytes follow via [`Handler::dcs_put`], terminated by
    /// [`Handler::dcs_unhook`].
    fn dcs_hook(&mut self, _params: &Params, _intermediates: &[u8], _ignore: bool, _action: char) {}

    /// A payload byte of the device control string most recently started with
    /// [`Handler::dcs_hook`].
    fn dcs_put(&mut self, _byte: u8) {}

    /// End of the current device control string.
    ///
    /// Also invoked when the DCS is aborted (e.g. by `CAN`/`SUB`).
    fn dcs_unhook(&mut self) {}

    /// Start of an application program command (APC).
    ///
    /// Payload bytes follow via [`Handler::apc_put`], terminated by
    /// [`Handler::apc_end`].
    fn apc_start(&mut self) {}

    /// A run of payload bytes of the application program command most
    /// recently started with [`Handler::apc_start`].
    ///
    /// The parser delivers payload in runs as large as the input allows, so a
    /// single APC may produce multiple calls.
    fn apc_put(&mut self, _bytes: &[u8]) {}

    /// End of the current application program command.
    ///
    /// Also invoked when the APC is aborted (e.g. by `CAN`/`SUB`).
    fn apc_end(&mut self) {}
}
    ...
    T: Timeout,
{
    ...
    fn opaque_hook(&mut self, kind: OpaqueSequenceKind) {
        self.state.opaque_sequence_kind = Some(kind);
        if kind == OpaqueSequenceKind::Apc {
    ...
    fn opaque_put_bytes(&mut self, bytes: &[u8]) {
        // SOS/PM payloads are consumed silently, matching the previous
        // behavior of the parser-level `SosPmApcString` state.
        if self.state.opaque_sequence_kind == Some(OpaqueSequenceKind::Apc) {
            self.handler.apc_put(bytes);
        }
    }

    #[inline]
    fn opaque_unhook(&mut self) {
        if mem::take(&mut self.state.opaque_sequence_kind) == Some(OpaqueSequenceKind::Apc) {
    ...
    fn osc_dispatch(&mut self, params: &[&[u8]], bell_terminated: bool) {
        ...
        match params[0] {
            ...
            b"1337" if params.len() >= 2 => {
                // The OSC parser splits on ';', so rejoin params[1..] to
                // reconstruct the full keyword=header:base64 payload and pass
                // it raw to the handler for iTerm-specific parsing.
                let joined: Vec<u8> = params[1..].join(&b';');
                self.handler.osc_1337_raw(&joined);
            },
    ...
    fn csi_dispatch(
        ...
        match (action, intermediates) {
            ...
            ('S', [b'?']) => {
                ...
                let pv = params_iter.next().map(|p| p[0] as u32).unwrap_or(0);
...
mod tests {
    ...
    enum DcsApcEvent {
        DcsHook { params: Vec<Vec<u16>>, intermediates: Vec<u8>, ignore: bool, action: char },
        DcsPut(u8),
        DcsUnhook,
        ApcStart,
        ApcPut(Vec<u8>),
    ...
    /// Handler recording only the DCS/APC plumbing calls.
    #[derive(Default)]
    struct DcsApcHandler {
        events: Vec<DcsApcEvent>,
    }

    impl Handler for DcsApcHandler {
        fn dcs_hook(&mut self, params: &Params, intermediates: &[u8], ignore: bool, action: char) {
            self.events.push(DcsApcEvent::DcsHook {
        ...
        fn dcs_put(&mut self, byte: u8) {
            self.events.push(DcsApcEvent::DcsPut(byte));
        }

        fn dcs_unhook(&mut self) {
            self.events.push(DcsApcEvent::DcsUnhook);
        }

        fn apc_start(&mut self) {
            self.events.push(DcsApcEvent::ApcStart);
        }

        fn apc_put(&mut self, bytes: &[u8]) {
            self.events.push(DcsApcEvent::ApcPut(bytes.to_vec()));
        }

        fn apc_end(&mut self) {
            self.events.push(DcsApcEvent::ApcEnd);
        }
    }

    #[test]
    fn dcs_reaches_handler() {
        let mut parser = Processor::<TestSyncHandler>::new();
        let mut handler = DcsApcHandler::default();

        parser.advance(&mut handler, b"\x1bP1;2+qAB\x1b\\");
        ...
        assert_eq!(handler.events, vec![
            ...
            DcsApcEvent::DcsPut(b'A'),
            DcsApcEvent::DcsPut(b'B'),
            DcsApcEvent::DcsUnhook,
        ]);
    }

    #[test]
    fn apc_payload_reaches_handler() {
        let mut parser = Processor::<TestSyncHandler>::new();
        let mut handler = DcsApcHandler::default();

        parser.advance(&mut handler, b"\x1b_Ga=q;\x1b\\");

        assert_eq!(handler.events, vec![
            DcsApcEvent::ApcStart,
            DcsApcEvent::ApcPut(b"Ga=q;".to_vec()),
            DcsApcEvent::ApcEnd,
        ]);
    }
    ...
    fn apc_split_across_advance_calls() {
        ...
        // The APC kind must survive `Performer` recreation between calls.
        parser.advance(&mut handler, b"\x1b_Ga=");
        parser.advance(&mut handler, b"q\x1b\\");

        assert_eq!(handler.events, vec![
            DcsApcEvent::ApcStart,
            DcsApcEvent::ApcPut(b"Ga=".to_vec()),
            DcsApcEvent::ApcPut(b"q".to_vec()),
            DcsApcEvent::ApcEnd,
        ]);
    }

    #[test]
    fn sos_pm_consumed_silently() {
        let mut parser = Processor::<TestSyncHandler>::new();
        let mut handler = DcsApcHandler::default();

        parser.advance(&mut handler, b"\x1bXsos payload\x1b\\\x1b^pm payload\x1b\\");
