...
impl<const OSC_RAW_BUF_SIZE: usize> Parser<OSC_RAW_BUF_SIZE> {
    ...
    pub fn advance<P: Perform>(&mut self, performer: &mut P, bytes: &[u8]) {
        ...
        while i != bytes.len() {
            match self.state {
                ...
                State::OpaqueString => i += self.advance_opaque_string(performer, &bytes[i..]),
                _ => {
    ...
    fn change_state<P: Perform>(&mut self, performer: &mut P, byte: u8) {
        match self.state {
            ...
            State::OpaqueEscape => self.advance_opaque_escape(performer, byte),
            // `Ground` and `OpaqueString` have dedicated bulk paths in the advance loops.
            State::Ground | State::OpaqueString => unreachable!(),
        }
    }
    ...
    fn advance_esc<P: Perform>(&mut self, performer: &mut P, byte: u8) {
        match byte {
            ...
            0x58 => {
                performer.opaque_hook(OpaqueSequenceKind::Sos);
            ...
            0x5E => {
                performer.opaque_hook(OpaqueSequenceKind::Pm);
                self.state = State::OpaqueString
            },
            0x5F => {
                performer.opaque_hook(OpaqueSequenceKind::Apc);
                self.state = State::OpaqueString
            },
            0x60..=0x7E => {
                performer.esc_dispatch(self.intermediates(), self.ignoring, byte);
    ...
    /// Advance the parser state from the opaque string state (SOS/PM/APC).
    ///
    /// Payload bytes are delivered in bulk: the input is scanned for the next
    /// byte requiring special handling (C0 controls and the C1 string
    /// terminator) and everything before it is forwarded as a single
    /// [`Perform::opaque_put_bytes`] run. This mirrors the [`memchr`]-based
    /// fast path used for the ground state; a predicate scan is used instead
    /// of [`memchr`] since the special byte set ("below 0x20 or 0x9C") cannot
    /// be expressed as a `memchr3` search without admitting control bytes
    /// into the payload.
    ///
    /// Returns the number of bytes consumed.
    #[inline]
    fn advance_opaque_string<P: Perform>(&mut self, performer: &mut P, bytes: &[u8]) -> usize {
        let num_bytes = bytes.len();
        let special = bytes.iter().position(|&b| b < 0x20 || b == 0x9C).unwrap_or(num_bytes);

        if special != 0 {
            performer.opaque_put_bytes(&bytes[..special]);
            if special == num_bytes {
                return num_bytes;
            }
        }

        let byte = bytes[special];
        match byte {
            0x07 => {
                performer.opaque_unhook();
                self.state = State::Ground
            },
            0x18 | 0x1A => {
                performer.opaque_unhook();
            ...
            0x1B => self.state = State::OpaqueEscape,
            0x9C => {
                performer.opaque_unhook();
                self.state = State::Ground
            },
    ...
    /// Handle the byte following an ESC inside an opaque string (SOS/PM/APC).
    ///
    /// `ESC \` (ST) terminates the string; any other byte means the ESC was
    /// part of the payload, so the ESC is delivered as payload and the
    /// current byte is processed as part of the string.
    #[inline(always)]
    fn advance_opaque_escape<P: Perform>(&mut self, performer: &mut P, byte: u8) {
        match byte {
            0x5C => {
                performer.opaque_unhook();
                self.reset_params();
            ...
            0x1B => {
                // The pending ESC was payload; the current one might begin ST.
                performer.opaque_put_bytes(&[0x1B]);
            },
            _ => {
                performer.opaque_put_bytes(&[0x1B]);
                self.state = State::OpaqueString;
                let _ = self.advance_opaque_string(performer, &[byte]);
            },
        }
    }
...
/// The kind of opaque sequence (SOS, PM, or APC) being dispatched.
...
pub trait Perform {
    ...
    /// Invoked when the beginning of an opaque sequence (SOS, PM, or APC) is
    /// encountered.
    ///
    /// Payload bytes follow via [`Perform::opaque_put_bytes`], terminated by
    /// [`Perform::opaque_unhook`].
    fn opaque_hook(&mut self, _kind: OpaqueSequenceKind) {}

    /// Invoked for a single payload byte of an opaque sequence (SOS/PM/APC).
    ///
    /// Only called through the default implementation of
    /// [`Perform::opaque_put_bytes`].
    fn opaque_put(&mut self, _byte: u8) {}

    /// Invoked with runs of payload bytes of an opaque sequence (SOS/PM/APC).
    ///
    /// The parser delivers all payload exclusively through this method, in
    /// runs as large as the input allows. The default implementation forwards
    /// each byte to [`Perform::opaque_put`], so implementors may override
    /// either method.
    fn opaque_put_bytes(&mut self, bytes: &[u8]) {
        for &byte in bytes {
            self.opaque_put(byte);
        }
    }

    /// Invoked when an opaque sequence (SOS/PM/APC) is terminated.
...
mod tests {
    ...
    impl Perform for Dispatcher {
        ...
        fn opaque_hook(&mut self, kind: OpaqueSequenceKind) {
            self.dispatched.push(Sequence::OpaqueHook(kind));
        }
        ...
        fn opaque_unhook(&mut self) {
            self.dispatched.push(Sequence::OpaqueUnhook);
        }
    }
    ...
    const ST_ESC_SEQUENCE: &[Sequence] = &[Sequence::Esc(Vec::new(), false, 0x5C)];
    ...
    fn expect_opaque_sequence(
        ...
        let mut expected = vec![Sequence::OpaqueHook(kind)];
        expected.extend(expected_payload.iter().map(|byte| Sequence::OpaquePut(*byte)));
        expected.push(Sequence::OpaqueUnhook);
        expected.extend(expected_trailer.iter().cloned());
    ...
    #[test]
    fn apc_c1_st_terminated() {
        expect_opaque_sequence(b"\x1b_Ga=q\x9cZ", OpaqueSequenceKind::Apc, b"Ga=q", &[
            Sequence::Print('Z'),
        ]);
    }
    ...
    fn sos_c0_st_terminated() {
        expect_opaque_sequence(
            b"\x1bXTest\x20\xFF;xyz\x1b\\AB",
            OpaqueSequenceKind::Sos,
            b"Test\x20\xFF;xyz",
            &[Sequence::Esc(Vec::new(), false, 0x5C), Sequence::Print('A'), Sequence::Print('B')],
        );
    }

    #[test]
    fn sos_c1_st_terminated() {
        expect_opaque_sequence(b"\x1bXTest\x9cA", OpaqueSequenceKind::Sos, b"Test", &[
            Sequence::Print('A'),
        ]);
    }
    ...
    #[test]
    fn pm_c1_st_terminated() {
        expect_opaque_sequence(b"\x1b^Test\x9cA", OpaqueSequenceKind::Pm, b"Test", &[
            Sequence::Print('A'),
        ]);
    }

    #[test]
    fn apc_embedded_esc_continues_payload() {
        expect_opaque_sequence(
            b"\x1b_a\x1bbc\x1b\\",
            OpaqueSequenceKind::Apc,
            b"a\x1bbc",
    ...
    #[test]
    fn apc_embedded_esc_fuzz() {
        // Any payload byte following an embedded ESC (except `\` and C1 ST)
        // must continue the payload, including the ESC itself.
        for follow in 0x20..=0xFFu8 {
            if follow == 0x5C || follow == 0x9C {
                continue;
            }

            let input = [0x1B, 0x5F, b'a', 0x1B, follow, b'b', 0x1B, 0x5C];
            expect_opaque_sequence(
                &input,
                OpaqueSequenceKind::Apc,
                &[b'a', 0x1B, follow, b'b'],
    ...
    #[test]
    fn apc_can_sub_abort() {
        for abort in [0x18, 0x1Au8] {
            let input = [0x1B, 0x5F, b'G', b'a', abort, b'Z'];
            let mut dispatcher = Dispatcher::default();
            let mut parser = Parser::new();
            ...
            assert_eq!(dispatcher.dispatched, vec![
                Sequence::OpaqueHook(OpaqueSequenceKind::Apc),
                Sequence::OpaquePut(b'G'),
                Sequence::OpaquePut(b'a'),
                Sequence::OpaqueUnhook,
                Sequence::Execute(abort),
                Sequence::Print('Z'),
            ]);
        }
    }
    ...
    #[test]
    fn apc_chunked_at_every_offset() {
        const INPUT: &[u8] = b"\x1b_Ga=q;AAAA\x1b\x40Z\x1b\\x";

        let mut dispatcher = Dispatcher::default();
        let mut parser = Parser::new();
        parser.advance(&mut dispatcher, INPUT);
        let expected = dispatcher.dispatched;
        ...
        for split in 1..INPUT.len() {
            ...
            assert_eq!(dispatcher.dispatched, expected, "mismatch for split at {}", split);
        }
    }

    #[test]
    fn opaque_put_bytes_bulk_delivery() {
        #[derive(Default)]
        ...
        impl Perform for BulkDispatcher {
            fn opaque_put_bytes(&mut self, bytes: &[u8]) {
        ...
        let mut dispatcher = BulkDispatcher::default();
        let mut parser = Parser::new();

        parser.advance(&mut dispatcher, b"\x1b_Ga=q;AAAAAAAA\x1b\\");

        assert_eq!(dispatcher.payload, b"Ga=q;AAAAAAAA");
        assert_eq!(dispatcher.runs, 1);
        assert!(dispatcher.unhooked);
    }
}
